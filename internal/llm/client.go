...
type Client struct {
	baseURL  string
	models   []string // primary + fallbacks
	apiKey   string
	hc       *http.Client
}
...
func New(baseURL, model, apiKey, apiKeyEnv string, fallbackModels ...string) *Client {
	key := apiKey
	if key == "" && apiKeyEnv != "" {
...
// On 429 (quota exhausted) it tries fallback models in order.
